"""Run with PYTHONPATH=src .venv/bin/python -m personal.demo.server."""
...
class Demo:
    ...
    def advance(self, count):
        ...
        for _ in range(min(count, len(self.events) - self.position)):
            next_event = event_from_mapping(self.events[self.position])
            self.accepted.append(self.engine.ingest(next_event))
            self.position += 1
    ...
    def act(self, request):
        ...
        with self.lock:
            ...
            if action == "reset":
                self.reset(
                    request.get("scenario", self.scenario),
                    request.get("capacity", self.capacity),
                )
            else:
                sid = request.get("shipment", self.shipment)
                if sid not in {event["shipment_id"] for event in self.events}:
                    raise ValueError("Choose a shipment from this stream.")

                checkpoint = utc(request.get("as_of", self.as_of.isoformat()))
                self.shipment = sid
                self.as_of = checkpoint

                if action == "step":
                    self.advance(request.get("count", 1))
                    if request.get("follow_time") and self.position:
                        last_event = self.events[self.position - 1]
                        self.as_of = utc(last_event["received_at"])
                        self.shipment = last_event["shipment_id"]
                    self.message = (
                        f"Processed {self.position:,} of {len(self.events):,} "
                        "deliveries in file order."
                    )
                elif action == "save":
                    ...
                    self.saved = {
                        ...
                        "as_of": self.as_of,
                        "wire": self.engine.score(self.shipment, self.as_of).to_wire(),
                    }
                    self.message = (
                        "Snapshot saved. Advance the stream, then restore to return "
                        "to this point."
                    )
                elif action == "restore":
                    ...
                    self.shipment = self.saved["shipment"]
                    self.as_of = self.saved["as_of"]
                    same = (
                        self.engine.score(self.shipment, self.as_of).to_wire()
                        == self.saved["wire"]
                    )
                    if not same:
                        raise RuntimeError(
                            "Restored prediction differs from the saved prediction."
                        )
                    self.message = (
                        "Restored. The serialized prediction matches the saved "
                    ...
                    target = (
                        self.artifact
                        if action == "reload_valid"
                        else self.work / "missing-model"
                    )
                    ...
                    self.message = (
                        "Same valid artifact reloaded. Prediction and telemetry are unchanged."
                        if success
                        else (
                            "Invalid reload rejected. The previous model still serves "
    ...
    def view(self):
        ...
        retained = next(
            (
                ...
                if shipment["shipment_id"] == self.shipment
            ),
            [],
        )
        ...
        latest = {
            ...
            for event in known_revisions(retained, self.as_of)
        }
        kept = {(event["event_id"], event["revision"]) for event in retained}
        deliveries = []
        seen = set()

        for index, event in enumerate(self.events[: self.position]):
            if event["shipment_id"] != self.shipment:
                continue
            ...
            if key in seen:
                ...
                status = "Not retained"
            elif utc(event["received_at"]) > self.as_of:
                status = "Not yet available"
            elif latest.get(event["event_id"]) != event["revision"]:
                status = "Superseded revision"
            elif utc(event["device_time"]) > utc(event["received_at"]):
                status = "Device clock excluded"
            elif event["kind"] != "temperature_c" or not isinstance(
                event["value"],
                (float, int),
            ):
                status = "Not a numeric temperature"
            elif utc(event["device_time"]) <= self.as_of - timedelta(hours=3):
        ...
        return {
            ...
            "shipments": list(dict.fromkeys(event["shipment_id"] for event in self.events)),
            "shipment": self.shipment,
            "as_of": self.as_of.isoformat(),
            "horizon_end": (self.as_of + timedelta(hours=6)).isoformat(),
            "lookback_start": (self.as_of - timedelta(hours=3)).isoformat(),
            "next_event": (
                self.events[self.position] if self.position < len(self.events) else None
            ),
            "last_event": self.events[self.position - 1] if self.position else None,
            "deliveries": deliveries[-200:],
            "displayed_deliveries": min(len(deliveries), 200),
            "shipment_deliveries": len(deliveries),
            "prediction": prediction,
            "features": features,
            "stats": self.engine.stats(),
            "retained_shipments": [
                shipment["shipment_id"] for shipment in state["shipments"]
            ],
            "has_snapshot": self.saved is not None,
                    ...
                    [
                        {
                            ...
                            "incident_at": "2026-01-01T13:00:00+00:00",
                            "label_available_at": "2026-01-02T09:00:00+00:00",
                        }
                    ]
                    if self.shipment == "shipment-A"
                    else []
                )
                return {
                    "note": "Invented incident for the teaching example; not used by the model.",
                    "reports": reports,
                }

            return {
                "note": (
                    "Retrospective audit reports from the supplied dataset. "
                    "No report does not establish a negative."
                ),
                "reports": [
                    report
                    for report in read_jsonl(self.data / "labels.jsonl")
                    if report["shipment_id"] == self.shipment
                ],
            }
    ...
    class Handler(BaseHTTPRequestHandler):
        ...
        def do_GET(self):
            ...
            try:
                if route == "/api/state":
                    ...
                    with current.lock:
                        self.send(200, current.view())
                elif route == "/api/evaluation":
                    evaluation = json.loads((demo.artifact / "evaluation.json").read_text())
                    self.send(200, evaluation)
        ...
        def do_POST(self):
            ...
            if origin and urlsplit(origin).netloc != self.headers.get("Host"):
                self.send(403, {"error": "Use the demo from its own local page."})
                return
            ...
            try:
                ...
                current = sessions.get(self.headers.get("X-Demo-Session"))
                self.send(200, current.act(request))
...
def main():
    ...
    try:
        ...
        parser.exit(
            1,
            (
                f"Could not start the demo: {exc}\n"
