...
These notes explain what I tried, what the results showed, and why I made each choice. The entries follow the order of the work: a limitation marked open in an early checkpoint may be resolved later. Checkpoint 15 records the completed engine. Interview notes are prompts to review, not claims that every concept has already been discussed.

## How to use this journal
After each checkpoint, I record what I tried, the result and the reason for my choice. I include failed experiments and revised choices. I use measured evidence when comparing models. My current priorities are in [my plan](plan.md), and my final technical choices are in [DECISIONS.md](../DECISIONS.md).
...
**Question:** How will I build the assignment while understanding the reasons behind each decision?
...
**Work:** I read the assignment, contracts, starter implementation, tests, and generator. Created personal/plan.md and personal/notebooks/01_learning_lab.ipynb. Executed the notebook's six code cells in order and saved their outputs.
...
**Limits:** Exploration consumes the assignment's seven-hour effort budget. Notebook experiments are not the final deliverable. I must later extract shared, tested feature logic so training and serving do not drift apart. No model or final feature set has been selected.
...
**My review:** I still need to practise explaining this checkpoint.

**Open questions:** How should I resolve revisions and bad clocks? What makes a negative label sufficiently observed? Which features add useful information?
...
**Limits:** Helper is for historical reconstruction with full input available, not online ingestion. It does not solve size limited retention, clock validity, outcome completeness, or production rejection behavior. No ML model has been selected or trained.
...
**Limitations:** No trained model, final clock tolerance, window policy, label completeness, or size limited online state. Old device clocks may still be misleading. Notebook execution used sequential Python, not a newly launched Jupyter kernel.
...
**Concepts:** Historical past period differs from the future six hour prediction window. Mean describes observed level, maximum describes peak, slope describes change per hour. Count and actual observation span describe available evidence. Missing slope is not zero slope.

**Choices and alternatives:** Historical window with its left boundary excluded (checkpoint minus width, checkpoint]; average that gives each reading equal weight; slope of the best fitting straight line on elapsed hours rather than row numbers. Endpoint slope is simpler but ignores interior data; time-weighted means or robust slopes merit consideration if bursts/outliers matter. Three hours is illustrative, not selected via evaluation.
...
## Checkpoint 5: Turn JSON into analysis tables
...
Review answer: an immature checkpoint should remain out of training. Clarification: exclusion follows my symmetric waiting rule; it can apply even when a positive is already known, not only when the outcome is unknown.
...
**Accepted choice:** Selected 48 hours as the working extra time for reports to arrive. It remains an assumed completeness policy, not a demonstrated SLA.

**Work:** I combined previously explored features with eligible labels, preserving decision time versus training cutoff. Copied helper definitions unchanged into the independent notebook; final production extraction remains pending. Explicitly selected nine X columns and kept y and metadata separate. Exported personal/data/tables/checkpoint08_training_rows.csv.
...
**Decisions/limits:** Three-hour feature window and 70% timestamp cutoff remain demonstration settings, not final selections. No model trained; no missing-value filling missing values fit. Current features include potentially redundant and constant columns, to review before fitting. No predictive performance claims. Final split and final test outcome coverage remain open.
...
The discussion correctly placed training before prediction. Clarification provided: exclude the label from X because it is the target answer, unavailable for a new prediction. Including it as a feature leaks the answer and produces misleading training/evaluation behavior. I do not discard labels during training: fit uses X as inputs and y as the separate target. Predict uses new X without its unknown y. Understanding of this specific reason is pending confirmation.
...
**Concepts:** Baseline, class imbalance, accuracy versus incident recall, training-frequency probability, and Brier score. A constant comparison model ignores X and learns one number from y. New prediction labels are not needed to produce scores.

**Evidence:** On the 1,209-row demonstration training subset, always-negative classification has 92.06% accuracy and 0% recall. Constant probability learned from training is 96/1209 = 7.9404%. Training Brier score is 0.073099 for the constant versus 0.079404 for zero probability. These are in-sample illustrations, not final test model-quality claims. Hand-calculated Brier and constant-rate assertions passed.

**Choice and limits:** Establish an interpretable constant comparison model before a feature-based model. Lower training Brier is expected because the mean minimizes squared error among constants. No ranking capability, final temporal split, filling missing values, or feature-based model has been established. Notebook recomputes reviewed training rows without relying on another notebook's state.
...
**Next:** Model concepts: feature weights and logistic probability mapping versus decision-tree questions; temporal evaluation and preparing model inputs before fitting candidate models.
...
**Evidence:** Four code cells verified. 309 mature final test rows from 104 shipments, 25 positives. Logistic AP 0.981277, Brier 0.003760, log loss 0.020847; constant AP 0.080906, Brier 0.074380. At illustrative 20% alert cutoff: 24 true positives, one false negative, zero false positives. Seven slices cover source, freshness, and trend missingness. Reliability bins and 300 shipment-bootstrap replicates recorded. Approximate AP interval .9392 to 1 (floating-point rounding can yield a tiny excess above one); Brier interval .000286 to .010140. Small synthetic period and assumed label completeness limit conclusions. No test-driven reselection or calibration.
**Artifacts:** Saved fitted input preparation steps and model, exact experimental feature runtime, feature order, dependencies, policy, checksums, and evaluation outputs. Notebook bundle is not yet the public training/serving contract. Discussion pending.

## Checkpoint 14: Fresh-process and artifact checks
**Starting point:** the selected model and artifact from checkpoint 13. No model changes or repeated final selection based on test outcomes. Three code cells verified. Two fresh processes from unrelated working directories exactly matched parent-process JSON with a fixed format for twelve probes, including feature fingerprints and probabilities. Duplicate deliveries, historical input permutation, and unavailable future correction left results unchanged. A damaged checksum was rejected. A different generated stream (seed 1730, 80 shipments, 1,467 deliveries) produced 240 valid predictions without retraining. This is handling of different inputs evidence, not new predictive validation.
**Limits:** These checks covered the notebook model only. The public engine, removal of shipment history, snapshots and concurrent reload were added in checkpoint 15. Shared helpers remain under personal/notebooks/support. Notebook phase is ready for review and subsequent Python implementation. Discussion pending.

## Checkpoint 15: From notebook experiment to executable risk engine

**Starting point:** notebooks 11–14 fixed the model family and verified its predictions. Operational guarantees still needed implementation; good model metrics alone do not provide limited memory, recoverable state or safe reload.

**Implementation choices:** training and live prediction paths share feature definitions. Serving uses validated JSON weights and input preparation settings rather than loading executable pickle. Shipment capacity and a record cap for each shipment bound history and its lookup index. Removal of shipment history means some old evidence is gone, so predictions disclose missing coverage. One lock gives each score a consistent state/model view; reload validates first and swaps the model without clearing telemetry. Snapshots preserve replay-relevant state and ordering, then replace the destination while preventing other calls from seeing a partial update.
...
The optional UI proposal follows one shipment through delivery, feature calculation and prediction. It also includes examples of a late correction, removal of shipment history, restore and failed model reload. It would call the existing engine rather than duplicate its rules. No UI has been built yet.
...
The demo follows one shipment through delivery, eligibility checks and prediction. It calls the existing engine and checks that the feature panel matches the prediction digest. Inspecting a snapshot keeps the display consistent with stored history after removal of shipment history. Audit outcomes are revealed separately and do not enter the feature calculation.
...
A reviewer needs a working startup command, an explanation of the evaluation split, comparison with a simple baseline, and evidence for each required behavior. The main README now puts those together. Average precision measures ranking; Brier score measures probability error. Slice counts matter: perfect recall on three incidents provides much less evidence than the same result on a large sample. Strong synthetic-data results do not settle reporting completeness or production performance. I should explain the rows still waiting for reports contract difference directly rather than imply every original requirement is met exactly.

## Checkpoint 18: what is still worth improving

The system passes 27 tests, but passing tests and meeting every interpretation of the assignment are different questions. The builder currently leaves checkpoints still waiting for reports out because their outcomes are unknown. The README asks for a row with a 0 or 1 answer at each checkpoint. This needs an explicit contract discussion rather than silently treating unknown as negative.

The other priorities concern evidence: a confirmed end of observation, measurements of memory and latency under load, and real data evaluation before selecting an operating threshold. A more complicated model is not the first improvement supported by the current results. The detailed review is in personal/WALKTHROUGH.md (Improvement priorities).
...
Replaced the manual playback dashboard with System checks. Each click demonstrates a specific guarantee using a small live example: size limited shipment count, exact snapshot recovery, and continued scoring after a rejected model reload. Full-stream replay remains a CLI task in personal/README.md.
...
A new seed creates another synthetic stream. Replaying it twice checks repeatable behavior; it does not prove accuracy on real data. Holding the seed fixed while changing capacity isolates the configuration change. Saved inputs, source fingerprints, predictions and snapshots let us investigate failures. Running tests in a fresh process checks local code edits; the API itself must restart to load edited engine modules.
...
Development order and presentation order serve different purposes. I learned through notebooks, but an interviewer can follow one message through the contract, ingest, time filtering, features and prediction before discussing training and evaluation. Each explanation should identify the choice, reason and test evidence. Personal materials now live under `personal/`; required deliverables remain at the root. The walkthrough also calls out limits, including assumed label completeness and exclusion of checkpoints still waiting for reports.
...
I audited paths after the move to `personal/`. Notebooks 11–14 now add repository `src/` explicitly before importing shared helpers. A kernel's display label does not establish its Python executable; use the project `.venv` interpreter. Added a temporary-copy checker that executes every notebook in a fresh kernel, including training and separate process artifact checks. All 14 notebooks and 28 core/demo tests passed. Saved submission artifacts were not regenerated.

## Checkpoint 25: CLI follow-up rehearsal

I expanded section 7 of the walkthrough around the five README follow-up tasks. Commands cover new stream replay, locating the first mismatch, per-delivery capacity checks, evaluation evidence, and snapshot/reload recovery. Verified seed 1927 with 80 shipments at capacities 32 and 8. Both replay pairs matched. A proposed CLI capacity-validation change was exercised only in a temporary copy: the new tests failed before the fix, all 23 core tests passed afterward, and valid replay bytes matched the original implementation. This rehearsal did not change the submission code or model. The exact interview failure and requested requirement remain unknown.

## Checkpoint 26: explain the code in plain English

I rewrote code explanations using everyday words and kept the argument names, return values and error details. Added missing explanations in the generator, notebook helpers and model checker. Simplified the decision record, walkthrough, run instructions and notebook teaching text. The original assignment text remains available unchanged. A comparison of parsed Python code confirmed that the calculations and program flow did not change. All 28 tests and all 14 notebooks passed. Saved results and models were left intact.

## Checkpoint 27: remove stale references and unused helpers

Checked local document links, section links, notebook links and HTML asset paths. Updated old presentation source paths during export so they cannot return on the next export. Removed the outdated UI proposal and external resume file dependency from the plan. Removed four notebook helper definitions that were replaced by the package imports, and removed the placeholder test that only asserted True. There are now 27 meaningful tests; the lower count reflects removal of the placeholder. Improvement priorities remain in walkthrough section 10.

Notebook 14 now imports timedelta directly instead of depending on the helper module to expose it. The link audit covered 47 local file/section links and 22 presentation file references.

## Checkpoint 28: clear customer decisions

Reordered DECISIONS.md around the ten customer suggestions. Each answer states Reject, explains the reason, and points to the implemented rule. Label assumptions, the unmet training-row requirement, model selection and work left out of scope are stated separately. This changed documentation only.
